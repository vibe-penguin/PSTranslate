...
`batch_size` 控制每次请求最多翻译多少个文字图层，`batch_max_chars` 控制单次请求里受保护文本的总字符数上限，`concurrency` 控制同时发送多少个批量请求。脚本不会对相同 `originalText` 去重，也不会使用翻译缓存；每个文字图层仍然会以自己的 `layerId` 独立进入批量请求。
...
回写仍然只按 `layerId` 匹配，不依赖返回顺序。Python 会按 `concurrency` 并发处理多个批次，但不会合并重复文本、不会使用缓存。若某个批次返回格式不正确、缺少 `layerId` 或占位符丢失，脚本会先重试；仍失败时会自动把该批次拆成更小批次，最后只跳过失败图层。
